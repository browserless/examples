...
curl -X POST \
  ...
  -d '{"url": "https://scraping-sandbox.netlify.app/clarity-health"}' \
  --output page.png
...
curl -X POST \
  ...
  -H "Content-Type: application/json" \
  -d '{
    "query": "mutation AcceptCookies { goto(url: \"https://scraping-sandbox.netlify.app/clarity-health\", waitUntil: networkIdle) { status } if(selector: \"[id*=accept], [class*=accept], button[id*=cookie]\", visible: true) { acceptBtn: click(selector: \"[id*=accept], [class*=accept], button[id*=cookie]\") { time } } screenshot { base64 } }"
  }'
